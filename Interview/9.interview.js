...
const sumOfArrayOfObjects = () => {
  ...
  const resObj = arr3.reduce((accumulator, current) => {
    ...
    if (!accumulator.hasOwnProperty(key)) {
      ...
      for (let item in current) {
        if (accumulator[key].hasOwnProperty(item)) {
          accumulator[key][item] += current[item];
        } else {
          accumulator[key][item] = current[item];
        }
      }
...
console.log(sumOfArrayOfObjects());
